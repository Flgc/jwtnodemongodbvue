 ...
 *
 * file: Whatsapp.js
 * Description: Responsável por consumir o backend das mensagens e clientes através do Wathsapp no frontweb
 * Data: 30/04/2021
 */
...
const useStyles = makeStyles((theme) => ({
  ...
  rightContainer: {
    background: 'url(/wall.png) bottom',
    flex: 1,
  },
  heightAdjust: {
    display: 'flex',
...
export default function WhatsApp() {
  ...
  const handleClickOpen = () => {
    if (contact.chatId) setOpen(true);
  };
  ...
  function isValidLast(message) {
    try {
      if (
        ...
        message.lastMessage.body !== undefined
      ) {
        let mess = String(message.lastMessage.body);
        ...
        {messagesList.map((message) => {
          switch (message.type) {
            case 'chat':
              return (function () {
                let classMessage =
                  message.isServer === true ? classes.sent : classes.received;
                ...
                <Card className={classes.card}>
                  <Grid container>
                    <Grid item xs={3}>
                      ...
                      <Paper elevation={2}>
                        <GridList
                          ...
                          cols={1}
                          cellHeight={(window.innerHeight / 4.5) * list.length}
                        >
                          <List className={classes.list}>{getLeftList()}</List>
                        </GridList>
                      </Paper>
                    </Grid>
                    ...
                    <Grid className={classes.heightAdjust} item xs={9}>
                            ...
                            <IconButton
                              onClick={async () => {
                                ...
                                if (contact.firstAttendace === false) {
                                  await api.put(
                                    '/api/clients/' + contact._id,
                                    data
                                  );
                                } else {
                                  await api.patch(
                                    '/api/clients/first/?_id=' + contact._id,
                                    data
                                  );
                                }
                      ...
                      <CardContent
                        className={[classes.rightContainer, classes.content]}
                        style={{ marginTop: '0%' }}
                      >
                        <Grid>{getRightList()}</Grid>
                      </CardContent>
                          ...
                          <Button
                            onClick={() => {
                              ...
                              getBase64(selectedFile).then(async (data) => {
                                ...
                                await api
                                  .post('/api/whatsapp/message.doc?id=0', dados)
                                  .then(() => handleClose);
                              });
                            }}
                            color="primary"
                      ...
                      >
                        {contact.firstAttendace ? (
                          ...
                          <Grid
                            style={{ paddingLeft: '5px', marginTop: '1%' }}
                            container
                            item
                            xs={1}
                          >
