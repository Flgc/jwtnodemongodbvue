 ...
 *
 * file: src/controllers/class/dialogflow.js
 * Description: Caixa de dialogo padrão google (trocaremos em breve para @Material)
 * Data: 01/06/2021
 */
...
const fallbackResponses = require('../../functions/exports/fallbackResponses');
const fs = require('fs');
...
module.exports = class {
  ...
  async sendText(query) {
    ...
    let intentResponse;
    try {
      intentResponse = await this.detectIntent(query, context);
  ...
  async sendTextResponseAudio(query) {
    ...
    let intentResponse;
    try {
      intentResponse = await this.detectIntentwAudio(query, context);
