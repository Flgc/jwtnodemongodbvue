 ...
 * Description: Responsável pelas classes do venom
 * Data: 01/06/2021
 */
...
const messageHelper = require('../../controllers/message.controllers');
const notifierHelper = require('../class/notifier');
const notifier = new notifierHelper();
const clientHelper = require('../client.controllers');
  ...
  async initVenom() {
    ...
    if (process.env.SEND_NO_PISHING !== '0') {
      await this.Client.sendText(
        this.#myself.number,
        auxFunctions.InitialMessage(this.#myself)[0]
      ).then(console.log('- [INITIAL_MESSAGE][0]: Sent'));
      await this.Client.sendText(
        this.#myself.number,
        auxFunctions.InitialMessage(this.#myself)[1]
      ).then(console.log('- [INITIAL_MESSAGE][1]: Sent'));
      await this.Client.sendText(
        this.#myself.number,
        auxFunctions.InitialMessage(this.#myself)[2]
      ).then(console.log('- [INITIAL_MESSAGE][2]: Sent'));
    }
    ...
    fs.unlink(
      path.resolve('./controllers/Classes/Temp/qrcode' + this.#index + '.png'),
      () => {}
    );
    ...
    setInterval(async () => {
      ...
      if (battery <= 5) {
        notifier.notify(
          'Bateria baixa, convém ligar o celular da sessão: ' +
            this.#index +
            ' ao carregador.'
        );
      }
  ...
  async execMessages(message) {
    ...
    try {
      let bot = new dialogflow(
        this.#GCP_PROJECT_NAME,
        path.resolve(this.#JSON_LOCATION),
        this.#LANGUAGE_CODE,
        message.from
      );
      ...
      if (!RequestMongo.Exists) {
        if (!this.#IntenalAwaiting.includes(message.from)) {
          ...
          await this.Client.reply(
            message.from,
            `Olá ${auxFunctions.Greetings()}, você ainda não está cadastrado em nosso sistema.`,
            message.id.toString()
          );
          await this.Client.sendText(
            message.from,
            'Para podermos lhe atender com uma experiência completa, digite seu nome e sobrenome.'
          );
          return;
        } else {
          if (message.type === 'chat') {
            ...
            await clientHelper
              ...
              .then(() => {
                fs.mkdir(
                  path.resolve('./', 'Uploads') + '/' + message.from,
                  { recursive: true },
                  () => {}
                );
              });
      ...
      if (User.inAttendace === true) {
        ...
        if (message.type == 'chat') {
          ...
          let fileName = auxFunctions.WriteFileMime(
            message.from,
            message.mimetype
          );
      ...
      if (
        message.type === 'chat' &&
        message.body.length >
          (process.env.CHAR_LIMIT_PER_MESSAGE
            ? process.env.CHAR_LIMIT_PER_MESSAGE
            : 256)
      ) {
        ...
        return this.Client.sendText(
          message.from,
          'Desculpe, essa mensagem é muito longa!'
        );
      }
      ...
      if (message.type === 'chat') {
        ...
        (message.hasMedia === true && message.type === 'audio') ||
        message.type === 'ptt'
      ) {
        const Buffer = await this.Client.decryptFile(message);
        ...
        try {
          if (response.queryResult.fulfillmentText) {
            ...
            let nameAudioResponse = auxFunctions.WriteFileEXT(
              message.from,
              'mp3'
            );
            ...
            this.Client.sendVoice(message.from, dirResponse)
              .then(() => {
